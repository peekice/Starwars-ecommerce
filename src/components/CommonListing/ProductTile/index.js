    ...
    return <div onClick={()=>router.push(`/product/${item._id}`)}>
        ...
        <div className="my-4 mx-auto flex w-10/12 flex-col items-start justify-between">
            <div className="mb-2 flex">
                ...
                {item.onSale === 'yes' ?
                    <p className="mr-3 text-sm font-semibold text-red-700">{`$${(item.price - item.price * (item.priceDrop / 100)).toFixed(2)}`}</p> : null}
                {
                    item.onSale === 'yes' ?
                        <p className="mr-3 text-sm font-semibold">{`- ${item.priceDrop}%`}</p> : null
                }
